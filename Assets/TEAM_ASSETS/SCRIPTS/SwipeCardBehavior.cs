...
public class SwipeCardBehavior : MonoBehaviour {
    ...
    {  
        return !bottomBounds(transform.position + translation).Exists(vertex =>
         {
             // Check if point inside either slider
             return leftSlider.GetComponent<Collider>().bounds.Contains(vertex) || rightSlider.GetComponent<Collider>().bounds.Contains(vertex);
         });
    }
